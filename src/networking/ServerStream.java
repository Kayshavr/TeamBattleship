...
//	private Controller controller;
	...
	public ServerStream(String nickname, int usersRequired, int maxCapacity, boolean rejoin) throws IOException {
					...
					if(incomingMsg != null)
					{
						...
						switch(incomingMsg.getMsgType())
						{
							case CONNECT:
							{
								...
								mReply.setTimestamp(helper.getCurrentTimestamp());
...
//								controller.enableStartGame(checkCanStartGame());
				...
				// when the server kicks a user, IOException is thrown because the thread which is listening, tries to read from the stream, but the socket has been closed from the other endpoint
...
//		this.controller.addToTextArea(msg);
	...
	{
		Message msg = new Message(MessageType.KICK, helper.getCurrentTimestamp(), kickNickname, "You have been kicked out from the room");
	...
	{
		Message msg = new Message(MessageType.DISCONNECT, helper.getCurrentTimestamp(), this.nickname, "Server room closed");
